...
<table class="table table-bordered bg-white">
  ...
  <tbody>
    <?php foreach (($items ?? []) as $row): ?>
      <tr class="step-color-orange">
        <td><?= (int)$row['id'] ?></td>
        <td><?= (int)($row['numero_pv'] ?? 0) ?>/<?= htmlspecialchars(substr((string)($row['annee'] ?? '2026'), -2)) ?></td>
        <td><?= htmlspecialchars($row['date_pv'] ?? '') ?></td>
        <td><?= htmlspecialchars($row['proprietaire_nom'] ?? '') ?></td>
        <td>
          <a href="/public/index.php?route=etape2.show&id=<?= (int)$row['id'] ?>">عرض</a>
        </td>
      </tr>
    <?php endforeach; ?>
  </tbody>
