...
CREATE TABLE `proces_verbaux` (
  ...
  `lieu_id` INT,
  `description_situation` LONGTEXT,
...
CREATE TABLE `echanges_cour` (
  ...
  `document_url` VARCHAR(500),
  `designation_expert` VARCHAR(300),
  `created_by` INT,
  `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE `rapports_experts` (
  `id` INT PRIMARY KEY AUTO_INCREMENT,
  `echange_cour_id` INT,
  `type_rapport` ENUM('تقرير اختيار اولي','تقرير اختبار نهائي'),
  `document_url` VARCHAR(500),
  `decision_patrimoine` VARCHAR(255),
  `date_visite` DATE,
...
CREATE TABLE `decisions_finales` (
  ...
  `type_decision` ENUM('قرار إخلاء','قرار هدم'),
  `date_decision` DATE,
  `document_url` VARCHAR(500),
  `details` LONGTEXT,
